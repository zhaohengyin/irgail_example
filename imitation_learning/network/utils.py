...
def cross_entropy_loss(pred, label, keepdim=False):
    if not keepdim:
        return nn.BCELoss(reduction='sum')(pred, label) / pred.size(0)
    else:
        return nn.BCELoss(reduction='none')(pred, label)
...
def build_mlp_gaussian(input_dim, output_dim, hidden_units=[64, 64],
              hidden_activation=nn.Tanh(), output_activation=None):
    layers = []
    units = input_dim
    for next_units in hidden_units:
        layers.append(GaussianLayer(0.3))
        layers.append(nn.Linear(units, next_units))
        layers.append(hidden_activation)
        units = next_units
